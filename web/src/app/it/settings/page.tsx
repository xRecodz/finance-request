...
export default function ItSettingsPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6 px-4 py-6 md:px-6">
    ...
    <section className="panel rounded-2xl p-5"><h2 className="text-lg font-bold">Password default</h2><p className="mt-1 text-sm text-sli-muted">Berlaku untuk akun baru dan reset selanjutnya. Akun yang sudah login tidak berubah. Nilai tersimpan sebagai hash dan tidak dapat ditampilkan ulang.</p><form className="mt-4 flex flex-wrap gap-2" onSubmit={event => { event.preventDefault(); void save("/api/settings/default-password", { password }, "password"); setPassword(""); }}><input type="password" className="input min-w-[220px] flex-1" value={password} onChange={event => setPassword(event.target.value)} minLength={6} placeholder="Password baru, minimal 6 karakter" required /><button disabled={Boolean(busy)} className="btn-primary rounded-xl px-4">Ubah Password Default</button></form></section>
  </div>;
}
