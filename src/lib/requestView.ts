...
export const requestDetailInclude = {
  ...
  disbursementOfficer: { select: userBriefSelect },
  disbursements: { orderBy: { disbursedAt: "asc" } },
  category: true,
...
export function serializeRequestList(row: RequestListRow) {
  return {
    ...
    disbursementOfficer: row.disbursementOfficer,
...
export function serializeRequestDetail(row: RequestDetail) {
  return {
    ...
    approver: row.approver,
    disbursementOfficer: row.disbursementOfficer,
    disbursements: row.disbursements.map((entry) => ({ id: entry.id, amount: toNumber(entry.amount),
      disbursedAt: entry.disbursedAt, reference: entry.reference, status: entry.status })),
    category: row.category,
