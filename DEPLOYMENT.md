...
5. Untuk rilis berikutnya: backup database, `git pull` dari `main` atau deploy commit/tag yang disetujui, `npm ci` untuk kedua aplikasi, `npx prisma migrate deploy`, build, restart service, lalu smoke test. Jangan menjalankan seed atau reset password admin pada database production yang sudah dipakai. Jangan jalankan `approvers:sync` atau `managers:sync` sebagai bagian deploy otomatis: skrip tersebut dapat mengubah role yang ada, dan sinkronisasi approver dapat menghapus akun serta pengajuan uji. Atur rute melalui Portal IT setelah rilis.
